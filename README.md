...
├── newimg(PIPS)/         # 加入LPIPS后的实验结果（重建图、损失曲线、评估指标）
...
以下为加入 LPIPS 感知损失（w_lpips=20）后的实验结果：

**原图 vs 重建图：**

| 原图 | 重建图（有LPIPS） |
|:----:|:----:|
| ![原图](newimg(PIPS)/real_image.png) | ![重建图](newimg(PIPS)/fake_image.png) |

**训练损失曲线：**

![损失曲线](newimg(PIPS)/loss.png)

**评估指标：**

| 指标 | 结果图 |
|------|--------|
| 编码距离分布 | ![编码距离分布](newimg(PIPS)/encoding_distance_histogram.png) |
| ROC 曲线 | ![ROC曲线](newimg(PIPS)/roc_curve.png) |
| 阈值性能分析 | ![阈值性能](newimg(PIPS)/threshold_performance.png) |
...
| ROC-AUC | 0.6012 |
| PR-AUC | 0.7664 |
| 正常样本距离均值 | 0.0438 ± 0.0166 |
| 异常样本距离均值 | 0.0634 ± 0.0433 |
| 分离度（均值差） | 0.0196 |
| 最佳阈值 (Youden) | 0.0702 |
| 最佳F1分数 | 0.7552 |
| 最佳F1召回率 | 0.9891 |
...
加入 LPIPS 感知损失后，重建图像的纹理和细节保留能力显著提升，但由于当前数据集规模较小（训练集仅224张）且异常类型多样（8种），模型的区分能力仍有提升空间：

- **ROC-AUC=0.6012**：区分能力有限，正常/异常样本的编码距离分布存在较大重叠
- **分离度仅0.0196**：正常样本均值0.0438，异常样本均值0.0634，差距较小
- **最佳F1=0.7552**（召回率98.9%）：可通过降低阈值实现高检出率，但假阳性率会相应升高

潜在改进方向：
- 增加训练数据量
- 调整损失权重（如增大 w_enc 强化编码区分度）
- 引入数据增强策略
